#!/usr/bin/env python3
"""
Step 05: Frame Extraction for Conversations Pipeline
Extracts video frames at appropriate intervals for conversational content analysis.
"""
...
class ConversationsFrameExtractor:
    ...
    def process_all_videos(self):
        ...
        if not videos:
            videos = metadata.get('download_details', [])  # Conversations pipeline structure
    ...
    def process_single_video(self, video_info: Dict[str, Any]):
        ...
        elif 'download_path' in video_info:  # Conversations pipeline structure
            video_path = video_info['download_path']
        
        # For conversations pipeline, construct path from video directory if not found
        if not video_path:
            video_dir = self.input_dir / f"video_{video_id}"
        ...
        # If duration is still 0, try to get it from the video file using FFmpeg
        if duration == 0:
            try:
        ...
        if not video_path or not Path(video_path).exists():
            logger.warning(f"Video file not found for {video_id}: {video_path}")
            return 'failed'
        ...
        # Fallback: Check if frames already exist and are complete
        ...
        # Extract frames
        frames_info = self.extract_frames(str(video_path), video_id, video_frames_dir, duration)
    ...
    def calculate_frame_times(self, duration: float) -> List[float]:
        ...
        if duration <= 0:
            # Default to 60 minutes if duration unknown (typical conversation length)
            duration = 3600
    ...
    def extract_single_frame(self, video_path: str, timestamp: float, 
        ...
        try:
            ...
            result = subprocess.run(
                ...
                capture_output=True, 
                text=True,
                timeout=280  # 2 minute timeout for problematic video segments
            )
            ...
            logger.warning(f"Frame extraction timed out for timestamp {timestamp} (after 2 minutes)")
            return False
        except Exception as e:
        ...
        try:
            ...
            extraction_summary = {
                ...
                'extraction_parameters': {
                    ...
                    'frame_interval_minutes': self.frame_interval / 60,
...
def main():
    ...
    try:
        ...
        extractor = ConversationsFrameExtractor(progress_queue)
