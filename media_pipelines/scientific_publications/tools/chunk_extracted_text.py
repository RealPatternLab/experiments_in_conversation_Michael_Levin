...
def extract_page_number_from_text(text: str) -> Optional[str]:
    """
    Extract page number from text if it contains page markers.
...
class SemanticTextChunker:
    ...
    def __init__(self, extracted_text_dir: Path, chunked_dir: Path, prompt_file: Path):
    ...
    def get_text_files_to_process(self, max_files: int = 5) -> List[Dict]:
        ...
        chunks_dir = Path("data/transformed_data/semantic_chunking")
        ...
        for text_file in files_to_process:
            try:
                ...
                # Create simple metadata structure (no database dependency)
                simple_metadata = {
                    'authors': 'Unknown',
                    'journal': 'Unknown',
                    'doi': None,
                ...
                files.append({
                    ...
                    'file_size': file_size,
                    'authors': simple_metadata['authors'],
                    'journal': simple_metadata['journal'],
                    'doi': simple_metadata['doi'],
                    'year': simple_metadata['year'],
                    'title': simple_metadata['title'],
                    'confidence_score': simple_metadata['confidence_score'],
                    'document_type': simple_metadata['document_type']
                })
...
"""
    ...
    def save_chunks_json(self, file_metadata: Dict, chunks_response: SemanticChunksResponse, gemini_response: str = ""):
        ...
        try:
            ...
            # Save to JSON file
            json_dir = Path("data/transformed_data/semantic_chunking")
    ...
    def run_chunking(self, max_files: int = 5, parallel_workers: int = 1) -> Dict:
        ...
        if successful > 0:
            ...
            json_count = len(list(Path("data/transformed_data/semantic_chunking").glob("*_chunks.json")))
...
def main():
    ...
    parser.add_argument(
        ...
        help="Directory containing extracted text files to chunk (default: data/transformed_data/text_extraction)"
    )
    parser.add_argument(
        "--output-dir",
        type=Path,
        default=Path("data/transformed_data/semantic_chunking"),
        help="Directory to save chunked JSON files (default: data/transformed_data/semantic_chunking)"
    )
    parser.add_argument(
        "--chunking-prompt",
        type=Path,
        default=Path("data/transformed_data/chunking_prompt.txt"),
        help="Path to the chunking prompt file (default: data/transformed_data/chunking_prompt.txt)"
    )
    parser.add_argument(
        "--max-files",
        type=int,
    ...
    try:
        chunker = SemanticTextChunker(args.input_dir, args.output_dir, args.chunking_prompt)
